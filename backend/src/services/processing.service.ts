...
export const processPOD = async (podId: string): Promise<void> => {
  ...
  try {
    ...
    if (isStructuredFile) {
      ...
      logger.info("Processing structured file", {
        podId,
        mimeType,
        originalName,
      });
    ...
    try {
      ...
      logger.error("Error updating POD status to FAILED", {
        podId,
        error: updateError,
      });
    }
 ...
 */
export const reprocessPOD = async (
  podId: string,
  ...
  try {
    ...
    for (const podId of podIds) {
      try {
        ...
        logger.error("Error processing POD in delivery", {
          deliveryId,
          podId,
          error,
        });
    ...
    logger.info(
      "Individual documents processed, starting delivery-level validation",
      {
        deliveryId,
      }
    );
    ...
    try {
      ...
      logger.error("Error updating delivery status to FAILED", {
        deliveryId,
        error: updateError,
      });
    }
 ...
 */
export const scheduleDeliveryProcessing = async (
  deliveryId: string
): Promise<void> => {
