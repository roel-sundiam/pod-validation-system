...
import { analyzeImageQuality, isAcceptableQuality } from './image-quality.service';
    ...
    const worker = await createWorker(language, 1, {
      logger: (m) => {
        if (m.status === 'recognizing text') {
          logger.debug(`Tesseract worker ${i}: ${m.status} ${(m.progress * 100).toFixed(0)}%`);
        }
      },
    });
 ...
 */
export const extractTextFromPDFDocument = async (pdfPath: string): Promise<OCRResult> => {
  try {
    ...
    if (hasText) {
      ...
      for (const imagePath of imagePaths) {
        try {
          ...
          logger.error('Error processing PDF page', { imagePath, error: pageError });
 ...
 */
export const extractText = async (filePath: string, mimeType: string): Promise<OCRResult> => {
  try {
